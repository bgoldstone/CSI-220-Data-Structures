...
public class BinarySearch {
	 ...
	 * SearchBinIterative()
	 *
	 * Search for the key within the array A from location left to right. Return the
	 * index where it is found or -1.
	 */
	 ...
	 * SearchBinRecursive()
	 *
	 * Search for the key within the array A from location i to j. Return the index
	 * where it is found or -1.
	 */

	public static int SearchBinRecursive(int[] A, int key, int left, int right) {
		while (left <= right) {
			int mid = (left + right) / 2;
			// if mid is the number return mid index.
			if (key == A[mid])
				return mid;
			// else move the mid to left or right.
			else if (A[mid] < key)
